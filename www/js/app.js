...
/** Send a typed message to all connected peers. Messages are queued
 *  if no peer is connected yet, or for a specific conn that hasn't
 *  opened; the queue is flushed when a connection opens. */
...
function handleSyncData(data, conn) {
  ...
  try {
    if (data.type === 'note-created' && data.note) {
      ...
      if (!local) {
        ...
        createNote(n.title || 'Untitled', n.content || '', /^#[0-9a-fA-F]{6}$/.test(n.color) ? n.color : '#ffffff', n.remind_at || null, data.done, data.sync_id);
        return;
      }
      ...
      for (const rn of data.notes) {
        ...
        if (isTombstoned(rn.sync_id)) continue;
        const match = notes.find(n => n.sync_id === rn.sync_id);
        if (match) {
          // Last-write-wins: only overwrite if remote is newer
          const remoteTs = rn.updated_at ? new Date(rn.updated_at).getTime() : Date.now();
  ...
  input.addEventListener('change', () => {
    ...
    reader.onload = (e) => {
      try {
        ...
        // Merge imported notes — dedup by sync_id first (the export
        // includes it, and it's the cross-device identity), then by
        // id as a fallback for legacy exports.
        let added = 0, skipped = 0, updated = 0;
        for (const n of imported) {
          if (!n.title) continue;
          const validColor = /^#[0-9a-fA-F]{6}$/.test(n.color) ? n.color : '#ffffff';
          const match = n.sync_id
            ? notes.findIndex(ex => ex.sync_id === n.sync_id)
            : (n.id ? notes.findIndex(ex => ex.id === n.id) : -1);
          if (match !== -1 && n.sync_id) {
            // Already have this sync_id — update fields in place
            Object.assign(notes[match], {
              title: n.title,
              content: n.content || '',
              color: validColor,
            ...
            // Legacy id match — same note on the same device
            skipped++;
          } else {
            ...
            notes.unshift({
              ...
              remind_at: n.remind_at || null,
              sync_id: n.sync_id || generateSyncId(),
              created_at: n.created_at || nowISO(),
              updated_at: n.updated_at || nowISO(),
            });
            // Broadcast to peers so they stay in sync
            const inserted = notes[0];
            syncSend('note-created', {
              note: {
                sync_id: inserted.sync_id,
                title: inserted.title,
                content: inserted.content,
                color: inserted.color,
                remind_at: inserted.remind_at,
                done: inserted.done,
        ...
        toast(parts.length ? `Imported: ${parts.join(', ')}` : 'Nothing to import');
      } catch (err) {
